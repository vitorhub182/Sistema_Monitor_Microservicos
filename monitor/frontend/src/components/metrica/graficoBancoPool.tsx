...
export function GraficoBancoPool(entrada: AmbienteGraficoProps) {
  ...
  React.useEffect(() => {
    async function fetchData() {
      try {
        const resposta = await getMetricaBancoPool(parametros);
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer config={dynamicChartConfig}>
              <LineChart accessibilityLayer data={dataRecharts}>
                <XAxis
                  dataKey="label"
                  tickLine={false}
                  tickMargin={10}
